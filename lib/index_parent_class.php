...
class index_parent_class{
    ...
    public function getFamily(){
        ...
        $stmt = $this->db->prepare("SELECT * FROM goal WHERE family_id = :family_id AND NOT user_id = :user_id order by goal_deadline desc");
        $stmt->bindParam(':family_id', $family_id);
        $stmt->bindParam(':user_id', $_SESSION["user_id"]);
        $stmt->execute();
        $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
    ...
    public function getPoint(){
        ...
        $stmt = $this->db->prepare("SELECT * FROM child_data WHERE family_id = :family_id AND NOT user_id = :user_id order by goal_deadline desc");
        $stmt->bindParam(':family_id', $family_id);
        $stmt->bindParam(':user_id', $_SESSION["user_id"]);
        $stmt->execute();
        $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
